...
pub fn zero_grad_and_update_embeddings(
    params: &[Tensor],
    lr: f32,
        ...
        for p in params {
            ...
            match &mut tape.nodes[node_id] {
                Node::Scalar(node) => {
                    ...
                    // Embedding parameter:
                    //
                    // Because Embeddings::new/load creates all
                    // embedding Tensor nodes consecutively,
                    // node_id - embedding_start is the exact
                    // flat_values index.
...
pub fn zero_grad_and_update_adam(
    params: &[Tensor],
    lr: f32,
    embeddings: &embeddings::Embeddings,
    first_moments: &mut [f32],
    second_moments: &mut [f32],
    beta1_correction_inv: f32,
    beta2_correction_inv: f32,
    mlp_parameter_start: usize,
    mlp_grad_scale: f32,
    embedding_grad_scale: f32,
    batch_normalization: f32,
) -> f32 {
    const BETA1: f32 = 0.9;
    ...
    TAPE.with(|t| {
        ...
        for param_index in 0..params.len() {
            ...
            let grad_scale =
                if param_index < mlp_parameter_start {
            ...
            let m =
                &mut first_moments[param_index];

            let v =
                &mut second_moments[param_index];
            ...
            *v =
            ...
            let m_hat =
                *m * beta1_correction_inv;

            let v_hat =
                *v * beta2_correction_inv;
        ...
        update_sq_sum.sqrt() as f32
    })
}

pub fn snapshot_parameter_values(
    params: &[Tensor],
    start: usize,
    end: usize,
) -> Vec<f32> {
    assert!(
        start <= end,
        "Invalid parameter snapshot range"
    );

    assert!(
        end <= params.len(),
        "Parameter snapshot range out of bounds"
    );
...
pub fn restore_parameter_values(
    params: &[Tensor],
    start: usize,
    end: usize,
    values: &[f32],
    ...
    assert!(
        start <= end,
        "Invalid parameter restore range"
    );

    assert!(
        end <= params.len(),
        "Parameter restore range out of bounds"
    );

    assert_eq!(
        values.len(),
        end - start,
        "Snapshot size does not match parameter range"
    );
